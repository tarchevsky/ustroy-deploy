...
export const useFormValidation = (fields?: { name?: string }[]) => {
  const isClient = typeof window !== 'undefined'

  // Генерируем defaultValues с вложенностью для имён с точками
  let defaultValues: any = {}
  if (fields) {
    fields.forEach((f) => {
      if (!f.name) return // пропускаем поля без name (например, info)
      if (f.name.includes('.')) {
        const [outer, inner] = f.name.split('.')
        if (!defaultValues[outer]) defaultValues[outer] = {}
        defaultValues[outer][inner] = ''
      } else {
